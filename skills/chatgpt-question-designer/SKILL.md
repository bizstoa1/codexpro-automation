...
- Regular web work defaults to `gpt-5.6` at the highest supported non-Pro reasoning tier. Pro is quota-limited and may be designed only after an explicit user request; never infer or auto-upgrade to it. Qualified Pro uses Oracle, `GPT-5.6 Sol` at the Pro effort, and capability-gated bounded-write DevSpace at the exact project root. Its mission authority must name exact relative write paths; capability v1 forbids commands and external state. `pro-attachment` uses exact snapshot attachments only for immutable/external or DevSpace-unreadable evidence; it is never an automatic fallback.
